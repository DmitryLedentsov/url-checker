...
class LinkChecker:
    ...
    def process_url(self, url):
        try:
            # Добавлена обработка перенаправлений с allow_redirects=True
            response = requests.get(url, headers=self.headers, timeout=self.timeout, allow_redirects=True)
            status_code = response.status_code
            
            # Вывод информации о перенаправлениях
            if response.history:
                for redirect in response.history:
                    print(f"Перенаправление: {redirect.url} -> {redirect.status_code}")
                print(f"Конечный URL: {response.url} - Статус: {status_code}")
            else:
            ...
            if status_code == 200:
                ...
                for link in soup.find_all('a', href=True):
                    ...
                    # Добавлена проверка на самоссылки и дубликаты в пределах страницы
                    if (self.is_valid_url(normalized_url) and 
                        normalized_url != url and  # Исключаем самоссылки
                        normalized_url not in links):  # Исключаем дубликаты
...
def main():
    ...
    parser.add_argument('--depth-limit', type=int, default=1000000, help='Максимальная глубина проверки')
